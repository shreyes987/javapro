...
class Logixscroll {
   ...
   Logixscroll(LogixCktCanvas owner, JScrollBar scroll) {
      this.owner = owner;
      this.scroll = scroll;
      items = new Vector();
      topItem = 0;
    
      gates = new LogixGates[3][4];
      for (int kind = 0; kind < 3; kind ++)
         for (int facing = 0; facing < 4; facing ++)
            gates[kind][facing] = new LogixGates(kind,facing);
      ...
      LogixCktNub in = new LogixCktNub(LogixCktNub.ONLEFT,0.0,true);
      in.boundingBox.reshape(0,0,13,10);
      items.addElement(in);
      
      LogixCktNub out = new LogixCktNub(LogixCktNub.ONLEFT,0.0,false);
   ...
   LogixCktIT checkMouse(int x, int y) {  // respond to mouse click; return item hit, if any
      int itemNum = topItem + y/width;
      if (itemNum >= items.size())
         return null;
      LogixCktIT item = (LogixCktIT)items.elementAt(itemNum);
      if (itemNum < 3 && x < 16 && y < (itemNum-topItem)*width + 17) {
         currentGate[itemNum]++;
         if (currentGate[itemNum] > 3)
            currentGate[itemNum] = 0;
         FloatRect r = ((LogixGates)items.elementAt(itemNum)).boundingBox;
         LogixGates newItem = gates[itemNum][currentGate[itemNum]];
         synchronized(owner) {
            boolean selected = ((LogixGates)items.elementAt(itemNum)).selected;
            newItem.selected = selected;
            if (selected)
               owner.selectedItem = newItem;
         }
         items.setElementAt(newItem,itemNum);
         if (newItem.facing == LogixGates.FACEDOWN || newItem.facing == LogixGates.FACEUP)
   ...
   void setSelection(LogixCktIT selectedItem) {
   ...
   synchronized void deleteItem(LogixCkt cir) {
   ...
   synchronized void unDeleteItem(LogixCkt cir, int oldTopItem) {  
      if (deletedItemPos < 0)
         return;
   ...
   synchronized void addItem(LogixCkt item) {
   ...
   synchronized void addItems(Vector list) {
      ...
      for (int i = 0; i < list.size(); i++) {
         LogixCkt cir = (LogixCkt)list.elementAt(i);
   ...
   synchronized void updateScroll() {
      ...
      for (int i = topItem; i < max; i++) {
          LogixCktIT it = (LogixCktIT)items.elementAt(i);
          if (i < 3 && ( ((LogixGates)it).facing == LogixGates.FACEDOWN || ((LogixGates)it).facing == LogixGates.FACEUP ))
      ...
      if (owner.selectedItem != null && owner.selectionInScroller && selectedItemNum >= 0) {
         if (selectedItemNum < topItem || selectedItemNum >= topItem + visibleItems)
            ...
            if (selectedItemNum < 3 && ( ((LogixGates)owner.selectedItem).facing == LogixGates.FACEDOWN || ((LogixGates)owner.selectedItem).facing == LogixGates.FACEUP ))
