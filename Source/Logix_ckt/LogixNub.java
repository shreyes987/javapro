...
abstract class LogixNub extends LogixCktIT {
   ...
   void drawWithLines(Graphics g) { 
      ...
      if (kind != OUTPUT)  // for outputs, destination lines are in containing circuit
         for (int i = 0; i < destination.size(); i++)
            ((Logixline)destination.elementAt(i)).draw(g);
   }
   void selectConnectedLines(boolean select) { 
      if (source != null)
         source.selected = select;
      for (int i = 0; i < destination.size(); i++)
         ((Logixline)destination.elementAt(i)).selected = select;
   }
   Rectangle getCopyOfBoundingBox(boolean addInLines) { 
      ...
      if (addInLines) {
         ...
         for (int i = 0; i < destination.size(); i++)
            r.add(((Logixline)destination.elementAt(i)).boundingBox);
      }
      r.grow(3,3);
      return r.getIntRect();
   }
...
class Tack extends LogixNub {
   ...
   void reshape(float x, float y, float width, float height) { 
      ...
      for (int i = 0; i < destination.size(); i++)
         ((Logixline)destination.elementAt(i)).setBoundingBox();
   }
...
class LogixCktNub extends LogixNub {
   ...
   void copyDataInto(LogixCktNub it) {
      it.selected = selected;
      it.on = on;
      it.reshape(boundingBox.x,boundingBox.y,boundingBox.width,boundingBox.height);
      it.connect_x = connect_x;
   ...
   LogixCktIT copy() {  // copied without source and destination
      LogixCktNub it = new LogixCktNub(side,absolutePosition,kind==INPUT);
      copyDataInto(it);
      return it;
   }
