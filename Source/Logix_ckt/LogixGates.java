...
public class LogixGates extends LogixCktIT {
   ...
   int kind,facing;
   LogixCktNub in1, in2, out;
   
   LogixGates(int typeCode, int faceCode) {  
      kind = typeCode;
      facing = faceCode;
      out = new LogixCktNub(faceCode,0.5F,false);
      if (typeCode == NOTGATE) {
         in1 = new LogixCktNub((faceCode+2)%4,0.5F,true);
      }
      else {
         in1 = new LogixCktNub((faceCode+2)%4,0.25F,true);
         in2 = new LogixCktNub((faceCode+2)%4,0.75F,true);
      }
   }
   ...
   void drawWithLines(Graphics g) { 
      ...
      for (int i = 0; i < out.destination.size(); i++)
         ((Logixline)out.destination.elementAt(i)).draw(g);
   }
   ...
   void selectConnectedLines(boolean select) { 
      ...
      for (int i = 0; i < out.destination.size(); i++)
         ((Logixline)out.destination.elementAt(i)).selected = select;
   }
   ...
   Rectangle getCopyOfBoundingBox(boolean addInLines) { 
      ...
      if (addInLines) {
         ...
         for (int i = 0; i < out.destination.size(); i++)
            r.add(((Logixline)out.destination.elementAt(i)).boundingBox);
      }
      r.grow(1,1);
      return r.getIntRect();
   }
   ...
   LogixCktIT copy() {  // copied without source and destination
      LogixGates it = new LogixGates(kind,facing);
   ...
   void delete(LogixCkt owner) {
      ...
      for (int i = 0; i < out.destination.size(); i++) {
         for (int j = 0; j < out.destination.size(); j++) {
            Logixline line = (Logixline)out.destination.elementAt(i);
